        ...
        t.Catch = function (e) {
            ...
            switch (e.number) {
                ...
                case 401:
                    ...
                    document.location = document.location.protocol + "//" + document.location.host + "/default.html?from=" + param;
                    return;
            }
            //if (e.message) {
            //    title = e.name;
...
Dictionary.prototype.forIn = function (code) {
    for (var key in this) {
        if (this[key].constructor.toString().indexOf("Function") > 0)
            continue;
        if (code(key, this[key]) == false)
            break;
    }
}
function on_ipType_changed(sender) {
    var span = document.getElementById("spanIpType");
    span.innerHTML = sender.checked ? chinese.static : chinese.dynamic;

    var networks = document.getElementsByName("network");
    for (var i = 0; i < networks.length; i++) {
        networks[i].disabled = !sender.checked
    }
