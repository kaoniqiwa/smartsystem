...
function UploadService() {
    ...
    var ReadyStateChange = function (xmlHttp) {
        if (!_this.ServiceDate) {
            _this.ServiceDate = new Date(xmlHttp.getResponseHeader("Date"));
        }
...
function AsynHttpService(contentType, timeout) {
    ...
    var ReadyStateChange = function () {
        if (!_this.ServiceDate) {
            ...
            if (dateHeader) {
                _this.ServiceDate = new Date(dateHeader);
                if (_this.OnServiceDateLoaded)
                    _this.OnServiceDateLoaded(_this.ServiceDate);
            }
        }
        //第五步：判断和服务器交互是否完成，还要判断服务器端是否正确返回数据
    ...
    this.OnServiceDateLoaded = function (date)
    { }
}
