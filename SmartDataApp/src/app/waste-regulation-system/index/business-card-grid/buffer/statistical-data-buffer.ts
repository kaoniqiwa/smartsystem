...
import { GarbageStationNumberStatistic, GarbageStationNumberStatisticV2, GetGarbageStationStatisticNumbersParamsV2 } from "../../../../data-core/model/waste-regulation/garbage-station-number-statistic";
...
export class StatisticalDataBufferService extends ListAttribute implements IBusinessService {
    ...
    async ancestorDivisions(ancestorId: string, divisionId?: string) { 
        ...
        if (divisionId) {
            ...
            if (!result) {
                ...
                const response = await this.divisionService.list(param).toPromise();
                result = response.Data.Data;
                this.cache.set(this.ancestorIdDivisions + Md5.hashStr(ancestorId), result);
            }
            return result;
        }
    }
    ...
    async stationNumberStatistic(stationIds: Array<string>) {
        ...
        if (!result) {
            ...
            this.cache.set(this.garbageStationStatisticNumberList + Md5.hashStr(stationIds.join(',')), result);
        }
        return result;
