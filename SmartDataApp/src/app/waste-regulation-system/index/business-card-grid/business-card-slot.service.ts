      ...
      for (let i = 0; i < this.views.length; i++) {
        for (let j = 0; j < this.views[i].list.length; j++) {
          this.views[i].list[j].view.timeSpan.nextRefreshTime.setSeconds(
            this.views[i].list[j].view.timeSpan.nextRefreshTime.getSeconds() +
              (j + 1) * this.views[i].list[j].view.timeSpan.interval
          );
        }
      }
    };
    ...
    const run = () => {
      ...
      for (let i = 0; i < this.views.length; i++) {
        for (let j = 0; j < this.views[i].list.length; j++) {
          if (
            Date.now() >=
            this.views[i].list[j].view.timeSpan.nextRefreshTime.valueOf()
          ) {
