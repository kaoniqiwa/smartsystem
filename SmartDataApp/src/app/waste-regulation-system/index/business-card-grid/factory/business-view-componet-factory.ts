...
import { ConverterFactory } from "./business-view-converter-factory";
...
export class BusinessViewComponetFactory {
  ...
  ): BusinessViewComponetConstructor;
  create(
    businessConfig: IBusinessConfig[],
    cardHost: CardDirective
  ): BusinessViewComponetConstructor;
  create(
    businessConfig: IBusinessConfig | IBusinessConfig[],
    cardHost: CardDirective
  ) {
    let constructor = new BusinessViewComponetConstructor();
    let card = this.cardContentFactory.create<CardComponent>(
      cardHost,
      CardComponent
    );
    ...
    const create_ = (businessConfig: IBusinessConfig) => {
      ...
      let business =
        this.statisticBusinessInjector.createBusiness(businessConfig);
      if (business && business.timeSpan) {
        business.timeSpan.interval = businessConfig.dataTime;
        business.timeSpan.nextRefreshTime.setSeconds(
          business.timeSpan.nextRefreshTime.getSeconds() +
            business.timeSpan.interval
        );
      }
      let content = this.cardContentFactory.create<BaseView<IViewModel>>(
        card.instance.cardHost,
        businessConfig.cardType
      );
