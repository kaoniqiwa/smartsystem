...
import {
  SRServer,
  SRServerAddress,
} from "../../../../data-core/model/aiop/sr-server";
...
export class SRServiceService {
  ...
  constructor(
    ...
    private srRequestSerivce: SRServiceRequestSerivce
  ) {
  ...
  delAddressForm(index: number) {
    if (this.addressForm.length > 1) {
      if (this.addressForm.length - 1 != index) {
        MessageBar.response_warning("请依次删除该项");
        return;
      }
      this.addressForm.splice(index, 1);
      /**该时候的选中表单 */
      if (index == this.addressTabSelectedIndex)
        this.addressTabSelectedIndex = 0;
    }
  }
    ...
    if (item.Name == "") {
      ...
      item.Address.map((x) => {
        ...
        if (x.IPAddress == "" || x.IPAddress.validIP() == false) {
          MessageBar.response_warning("IP地址格式不对");
          check = false;
        } else if (x.Port == "" || x.Port.validIPPort() == false) {
          MessageBar.response_warning("端口号格式不对");
          check = false;
        }
      });
    }
