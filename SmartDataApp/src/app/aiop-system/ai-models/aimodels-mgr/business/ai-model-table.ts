...
export class AIModelsTable
  extends ResourcesTable
  implements IConverter, IPageTable<CameraAIModel>
{
  ...
  Convert<CameraAIModels, CustomTableArgs>(
    input: CameraAIModels,
  ...
  toTableModel(item: CameraAIModel) {
    ...
    tableField.updateTime = this.datePipe.transform(
      item.UpdateTime,
      "yyyy-MM-dd hh:mm"
    );
    tableField.labelIcon = this.ai_icon
      ? l.imgUrlRoot + l.aiModelIcon + this.ai_icon[item.Label]
      : "";
  ...
  editItem(item: CameraAIModel) {
    ...
    findVal.updateTime = this.datePipe.transform(
      item.UpdateTime,
      "yyyy-MM-dd hh:mm"
    );
    findVal.modelType = item.ModelType;
    findVal.labelIcon = this.ai_icon
      ? l.imgUrlRoot + l.aiModelIcon + this.ai_icon[item.Label]
      : "";
  }
}
