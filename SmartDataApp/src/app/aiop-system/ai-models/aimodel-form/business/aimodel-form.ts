...
} from "../../../../data-core/model/aiop/camera-ai-model";
...
export class AIModelFormService extends ListAttribute {
    ...
    const change = (items: CameraAIModelDTOLabel[], id_index: string[]) => {
      items.map((x) => {
        if (x.LabelId == id_index[0]) {
          ...
          else if (id_index.length == 1) x.LabelModelValue = inputVal;
        }
      });
    };
    if (inputVal && item) {
      ...
      if (this.editItem)
        this.editItem.ModelDTO.Labels.map((m1) => {
          if (m1.LabelId == id_index[0]) m1.LabelModelValue = inputVal;
          change(m1.Labels, id_index);
        });
      else if (this.parseItem)
        this.parseItem.ModelDTO.Labels.map((m1) => {
          if (m1.LabelId == id_index[0]) m1.LabelModelValue = inputVal;
          change(m1.Labels, id_index);
        });
    }
  ...
  constructor(
    private requestService: AIModelRequestService,
  ...
  checkForm(item: FormField) {
    if (item.ModelName == "") {
      ...
      MessageBar.response_warning("请上传模型文件");
      return false;
    } else if (this.modelIcons.find((x) => x.checked) == null) {
      MessageBar.response_warning("请选择模型图标");
      return false;
    }
    return true;
  }
  ...
  async modelIconsData() {
    ...
    if (response) {
      for (const key in response) {
        this.modelIcons.push(
          new PicturesDropList(
            key,
            this.imgUrlRoot + this.aiModelIcon + response[key]
          )
        );
      }
    }
  ...
  async saveFrom(
    ...
    const check = this.checkForm(item),
      save = async (model: CameraAIModel) => {
    ...
    if (check) {
      ...
      if (this.formState == FormStateEnum.create) {
        ...
        if (this.parseItem) {
          ...
          this.parseItem.Label = Number.parseInt(
            this.modelIcons.find((x) => x.checked).id
          );
          save(this.parseItem);
        } else {
          save(model);
        ...
        //后台要求
        if (item.FileModelName) model.ModelDTO = null;
        else model.ModelJSON = "";
        const response = await this.requestService.set(model).toPromise();
        if (response.FaultCode == 0) {
          MessageBar.response_success();
          if (item.FileModelName) {
            const responseDto = await this.requestService
              .parse(item.ModelJSON)
              .toPromise();
          ...
          } else successFn(true, response.Data, this.formState);
        }
      }
    }
  }
