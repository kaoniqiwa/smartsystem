...
export class InputLabelService {
  ...
  bindLabel(
    ...
    const bind = items.filter(
      (x) => x.type == 1 && x.checked == true && x.del == false
    );
      ...
      if (this.findResourceFn) {
        ...
        if (item) {
          for (const x of bind) {
            ...
            if (index == -1) {
              const response = await this.resourceLabelRequestService
                .create(item.Id, x.id)
                .toPromise();
  ...
  async forBindLabelForm(
    tableSelectId: string,
    ...
    if (tableSelectId) {
      for (const x of bind) {
        ...
        if (index == -1) {
          await this.resourceLabelRequestService
            .create(tableSelectId, x.id)
            .toPromise();
        }
      }
      for (const x of unbind) {
