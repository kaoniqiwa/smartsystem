...
export class AICameraPanel extends ListAttribute {
  ...
  addAIModelToCameraFn: (
    cameraId: string,
    aiModelId: string,
    successFn: (success: boolean) => void
  ) => void;
  delAIModelToCameraFn: (
    cameraId: string,
    aiModelId: string,
    successFn: (success: boolean) => void
  ) => void;
  ...
  addCards(
    cameraId: string,
    model: { id: string; label: string; icon: string }
  ) {
    const item = this.cardListPanelView_.listPanel.find(
      (x) => x.id == cameraId
    );
  ...
  set cardListPanelView(items: AiopCamera[]) {
    items.map((x) => {
      ...
      x.Labels.map((l) => {
        const icon = new AccessoryIcon(),
          iconLabel = new AccessoryIconLabel();
      ...
      icon.icon =
        x.OnlineStatus == 0 ? "howell-icon-signal2" : "howell-icon-no_signal";
      card.barOtherAccessorys.push(icon);
      ...
      this.findCameraAIModel(x.Id, (models: CameraAIModel[]) => {
        models.map((m) => {
          card.barBody.push({
            ...
            label: m.ModelName,
            icon:
              this.imgUrlRoot + this.aiModelIcon + "ai-m" + m.Label + ".png",
          });
