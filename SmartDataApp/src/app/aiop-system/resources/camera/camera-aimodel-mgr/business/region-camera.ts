...
export class RegionCamera {
  ...
  tagCameraId = "";
  cameraAIModelCopyToFn: (
    tagCameraId: string,
    targetCameraId: string[]
  ) => Promise<boolean>;
  loadRegionCameraTree(cameras: AiopCamera[], treeNode: TreeNode) {
  ...
  cameraAIModelCopyTo(allCameraNodes: FlatNode[], callBack: () => void) {
    ...
    allCameraNodes
      .filter((c) => c.checked && c.id.indexOf(this.treeTypeTage) > -1)
      .map((x) => {
        const cameraId = x.id.replace(this.treeTypeTage, "");
        if (cameraId != this.tagCameraId) targetCameraIds.push(cameraId);
      });

    if (this.tagCameraId && targetCameraIds.length) {
      const success = this.cameraAIModelCopyToFn(
        this.tagCameraId,
        targetCameraIds
      );
      if (success) callBack();
