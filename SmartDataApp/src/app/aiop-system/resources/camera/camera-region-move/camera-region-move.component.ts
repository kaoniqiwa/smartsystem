...
import { TreeListMode, FlatNode, TreeNode } from '../../../../shared-module/custom-tree/custom-tree';
...
export class CameraRegionMoveComponent implements OnInit {
  ...
  async onSubmit() {
    if ((this.selectedNode && this.isLastNode) || (this.selectedNode&&this.selectedNode.id=='')) {
      for (const c of this.cameraIds) {
        const item = this.tableService.findCamera(c);
  ...
  get isLastNode() {
    var is = false;
    const nodes = this.tableService.regionTree.allLastChilds(this.treeDataSource);
