  ...
  implements FormAttribute
{
  ...
  async defaultForm(editItem: EncodeDevice) {
    ...
    if (editItem) {
      ...
      for (const item of response.Data) {
        // this.protocolTypes.push(item.ProtocolType);
        if (item.Url.indexOf("http") > -1) transType = TransTypeEnum.TCP;
        else if (item.Url.indexOf("tcp") > -1) transType = TransTypeEnum.UDP;
        this.form.patchValue({
          Name: item.Name,
  ...
  async saveFrom(
    ...
    if (check) {
      ...
      if (item.TransType) dev.TransType = Number.parseInt(item.TransType);
