...
export class BusinessService {
    ...
    constructor() {      
        ...
        this.table.updateItemFn = (item: GarbageStation) => {
            const findItem = this.divisionStationDataService.garbageStations.find(x => x.Id == item.Id);
            if (findItem) {
                for (var key in item)
...
export class GarbageStationTable extends BusinessTable implements IConverter, IPageTable<GarbageStation> {
    ...
    Convert<GarbageStations, CustomTableArgs>(input: GarbageStations, output: CustomTableArgs) {
