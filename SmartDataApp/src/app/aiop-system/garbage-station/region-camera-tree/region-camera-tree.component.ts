...
export class RegionCameraTreeComponent implements OnInit {
  ...
  selectedItemClick = (item: RightButtonArgs) => {
    if (this.selectedItemFn) this.selectedItemFn(item.node);
  };
  @Input()
  reloadStateFn: () => void;

  searchTree = (text: string) => {
    const dataSource = this.regionCameraTree.filterNodes(
      text,
      NodeTypeEnum.camera
    );
  ...
  constructor(
    private regionCameraTree: RegionCameraTree,
  ...
  async ngOnInit() {
    ...
    this.dataService.cameras = await this.dataService.allCamera(
      this.dataService.regions
    );
  ...
  findBindCameraNode(use: boolean) {
    ...
    for (let key of this.cameraTree.flatNodeMap.keys()) {
      if (
        key.iconClass == "howell-icon-video" &&
        key.rightClassBtn.length == 0 &&
        use
      )
        nodes.push(key);
      else if (
