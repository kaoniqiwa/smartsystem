...
export class DeployCameraComponent implements OnInit {
  ...
  selectDivisionClick = async (item: FlatNode, lastNode: boolean) => {
    const station = this.stationTree.dataService.garbageStations.find(
      (x) => x.Id == item.id
    );
    ...
    if (station) {
      ...
      if (type) {
        this.chartComponent.stationChart.changeTrashNum(
          type.Windows.length + ""
        );
        this.chartComponent.stationChart.changeHouseType = type;
        await this.businessService.fillHouseCameraPostion(
          this.chartComponent.stationChart.house,
          station.Id
        );
        // this.reloadState();
      }
  ...
  selectCameraClick = async (item: FlatNode) => {
    ...
    this.businessService.bindingCamera =
      this.cameraTree.dataService.cameras.find((x) => x.Id == item.id);
  };

  bindingItemClick = (liItem: {
    id: string;
    name: string;
    position: number;
    no: number;
  ...
  async findStationType(stationType: number) {
    if (this.typeDataService.types == null)
      this.typeDataService.types =
        await this.typeDataService.requestGarbageStationType();
