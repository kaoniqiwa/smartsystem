...
export class StationChart {
  ...
  trashSelected_: { door: string; trash: string; icon: string; type: CanType };
  /**垃圾桶 摄像机槽位下标 */
    ...
    const slots = new Array<CameraSlot>(),
      ...
      fillSlots = (arr: FormArray, outSide: boolean, moreWindow: boolean) => {
        ...
        for (const x of arr.controls) {
          ...
          if (outSide) slot.CameraUsage = CameraUsageData.Other;
          else {
            slot.Windows.push(i);
            /** 顺序编号 */
            if (moreWindow) slot.Windows.push(i + 1);

            if (
              this.positionNoMap.get(CanType.Dry) == x.value ||
              this.multifacetedPostion == x.value
            )
              slot.CameraUsage = CameraUsageData.Dry;
            else if (this.positionNoMap.get(CanType.Wet) == x.value)
              slot.CameraUsage = CameraUsageData.Wet;
            else slot.CameraUsage = CameraUsageData.Recycle;
          }
    ...
    if (type) {
      ...
      for (let i = 0; i < type.CameraSlots.length; i++) {
        ...
        if (i <= 3) {
          ...
          this.house.cameraPosition.inside.li1[i - 4].position =
            slot.PositionNo;
        }
        // else {
  ...
  createCameraPostionForm(size: number = 4) {
    ...
    for (let i = 11; i < 11 + size; i++)
      li1.push(this.fb.control(this.positionNoMap.get(CanType.Dry)));
  ...
  createHouse(size: number = 4) {
    const house = new House(),
      outSideMax = 11;
    const initInterior = () => {
        house.interior.push(this.getDoorTrash(CanType.Dry));
      ...
      camera = () => {
        ...
        for (let i = outSideMax; i < outSideMax + size; i++) {
          house.cameraPosition.inside.li1.push({
            id: "",
            name: "",
            position: 11,
            no: no,
  ...
  //         if (this.house.cameraPosition.inside.li1[i].position == this.positionNoMap.get(CanType.Dry) &&
  //             this.house.cameraPosition.inside.li1[i + 1].position == this.positionNoMap.get(CanType.Dry)) {
  ...
  set trashSelected(index: number) {
    ...
    /**勾选图标 */
    this.trashIcons.map(
      (x) => (x.selected = x.doorTrashs.door == this.trashSelected_.door)
    );
  }
  ...
  get configGarbageTrash() {
    ...
    for (var i = 4; i <= 5; i++) trashs.push(i);
    return trashs;
  }
  ...
  get insideGroup1LineSquareW() {
    const baseW = 498,
      ...
      baseNum = 4;
    return (
      baseW + (this.house.cameraPosition.inside.li1.length - baseNum) * poor
    );
  }
...
export class House {
  interior: { door: string; trash: string; icon: string; type: CanType }[] =
    new Array();
  cameraPosition: {
