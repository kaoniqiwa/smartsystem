...
export class RegionMgrComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.regionMgrService.regionTree.loadTree(this.regionMgrService.dataSource);
    this.tree.dataSource.data = this.regionMgrService.regionTree.dataSource;
    this.regionMgrService.regionTree.addTreeNode = (
      node: FlatNode,
      addNode: TreeNode,
      item: Region
    ) => {
      ...
      if (this.regionMgrService.noChildNodes(item.ParentId, 1)) {
        this.resetTreeNodes();
      } else this.tree.addItem(node, addNode);
      this.tree.defaultItem(item.ParentId);
      this.tree.treeControl.expandAll();
    };

    this.tree.defaultItem();
    this.regionMgrService.regionTree.editNodeItem = (
      node: FlatNode,
      val: string
    ) => {
  ...
  async delBtnClick() {
    const item = this.regionMgrService.regionTree.findRegion(
      this.regionMgrService.regionTree.selectedNodeId
    );
    const success = await this.regionMgrService.delRegionData(
      this.regionMgrService.regionTree.selectedNodeId
    );
    if (success) {
