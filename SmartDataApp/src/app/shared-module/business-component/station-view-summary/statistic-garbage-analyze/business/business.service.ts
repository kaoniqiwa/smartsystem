...
export class BusinessService extends ListAttribute {
    ...
    async requestData() {
        ...
        const statisticNumbers2 = await this.garbageStationService.statisticNumberListV2(param)
            .toPromise(); console.log(statisticNumbers2);
    ...
    statisticGroupBy(statistic: Array<GarbageStationNumberStatisticV2>){
        if (statistic) {         
            const groupData = new Map<string, Array<GarbageStationNumberStatisticV2>>();  
    ...
    convertLineData(statistic: Array<GarbageStationNumberStatisticV2>, search: SearchControl) {
        ...
        this.lineChartOption.axisLabel.formatter = (v: any) => {
            if (s.ClassType == CategoryNameEnum.IllegalDrop
                || s.ClassType == CategoryNameEnum.MixedInto)
                return v + '起';
            else if (s.ClassType == CategoryNameEnum.GarbageRatio)
                return v + '分';
            else
                return v + '分钟';
        }
                ...
                gd.map(g => {
                    ...
                    switch (s.ClassType) {
                        ...
                        case CategoryNameEnum.IllegalDrop:
                            data.push(findIllegalDrop(g.EventNumbers, EventTypeEnum.IllegalDrop).DayNumber + '');
                            break;
                        case CategoryNameEnum.MixedInto:
                            data.push(findIllegalDrop(g.EventNumbers, EventTypeEnum.MixedInto).DayNumber + '');
                            break;
    ...
    convertBarData(statistic: Array<GarbageStationNumberStatisticV2>, search: SearchControl) {
        const s = search.toSearchParam();
        ...
        if (statistic) {
            ...
            for (const key of groupData.keys()) {
                var gd = groupData.get(key), name = '',id ='';
                const data = new Array<string>();
                gd.map(g => {
                    if (id == '') {
                        name = g.Name;
    ...
    getRequsetParam(search: SearchControl): GetGarbageStationStatisticNumbersParamsV2 {
        const s = search.toSearchParam();
        ...
        if (s.TimeUnit == TimeUnitEnum.Week) {
            ...
            const date = new Date(Number.parseInt(s.Year)
                , Number.parseInt(s.Month) - 1, 1, 0, 0, 0, 0)
                , lastDay = MonthLastDay(Number.parseInt(s.Year), Number.parseInt(s.Month))
 ...
 exportExcel(statistic: Array<GarbageStationNumberStatisticV2>, search: SearchControl)
    : {
        table: XlsxData,
        chart: ExcelData,
        categoryName:string,
        ...
        for (const val of  groupData.get(key).values()) {
            ...
            switch (s.ClassType) {
                ...
                case CategoryNameEnum.GarbageDuration:
                    ...
                    dataUnit = '分钟';     
                case CategoryNameEnum.IllegalDrop:
                    td[i].val = findIllegalDrop(val.EventNumbers, EventTypeEnum.IllegalDrop).DayNumber;
                    dataUnit = '起';     
                    break;
                case CategoryNameEnum.MixedInto:
                    td[i].val = findIllegalDrop(val.EventNumbers, EventTypeEnum.MixedInto).DayNumber;
                    dataUnit = '起';     
                    break;
                default:
                    break;
