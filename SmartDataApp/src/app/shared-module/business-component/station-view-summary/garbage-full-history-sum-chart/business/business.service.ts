...
import { TheBeforeDate, OneWeekDate, MonthLastDay, TheDayTime, TimeInterval } from "../../../../../common/tool/tool.service";
...
export class BusinessService extends ListAttribute {
    ...
    async requestData() {
        ...
        if (param.GarbageStationIds.length) {
            ...
            data.items = statisticNumbers2.Data;
    ...
    getRequsetParam(search: SearchControl): GetGarbageStationStatisticNumbersParamsV2 {
        ...
        param.GarbageStationIds = sp.StationId && sp.StationId.length ? sp.StationId.split(',') : new Array();
        if (s.TimeUnit == TimeUnitEnum.Day) {
            const time = TimeInterval(new Date(Number.parseInt(s.Year)
                , Number.parseInt(s.Month) - 1, Number.parseInt(s.Day)).toISOString(),0,0,0,-1);
            param.TimeUnit = 2;
            ...
            const time = TheBeforeDate(new Date(Number.parseInt(s.Year)
                , Number.parseInt(s.Month) - 1, 1), 0, -1)
                , lastDay = MonthLastDay(Number.parseInt(s.Year), Number.parseInt(s.Month));
            param.TimeUnit = 4;
            param.BeginTime = time.begin.toISOString();
            param.EndTime = new Date(Number.parseInt(s.Year), Number.parseInt(s.Month) - 1, lastDay).toISOString();
        }
