...
import { GetGarbageDropEventRecordsParams, GarbageDropEventRecord } from "../../../../../data-core/model/waste-regulation/garbage-drop-event-record";
...
export class BusinessService {
    ...
    constructor(private datePipe: DatePipe
        ...
        this.galleryTargetView.neighborEventFnI = (ids, e: ImageEventEnum) => {
            const idV = ids.split('&'), findEvent = this.dataSource.find(x => x.EventId == idV[0])
                , cameras = ([...findEvent.Data.DropImageUrls, ...findEvent.Data.HandleImageUrls, ...findEvent.Data.TimeoutImageUrls] as Array<HWCameraImageUrl>);
        ...
        this.table.initGalleryTargetFn = (eventId, cameras, index) => { 
            this.galleryTargetView.initGalleryTargetI(eventId, cameras as Array<HWCameraImageUrl>, index);
        }
    ...
    getRequsetParam(pageIndex: number, search: SearchControl) {
        ...
        const param = new GetGarbageDropEventRecordsParams(), day = TheDayTime(new Date());
