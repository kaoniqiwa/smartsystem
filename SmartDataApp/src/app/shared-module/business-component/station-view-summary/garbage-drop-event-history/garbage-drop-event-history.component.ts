...
@Component({
  ...
  providers:[HWVideoService,BusinessService,DivisionDao,GarbageStationDao,GarbageStationCameraDao]
})
export class GarbageDropEventHistoryComponent implements OnInit {
  ...
  constructor(private tableService: BusinessService
  ...
  changeDivisionFn = (divisionId: string) => {
    if (divisionId) {
      const garbageStations = this.tableService.stations.filter(x => x.DivisionId == divisionId);
      this.tableService.search.toStationsDropList = garbageStations;
      const resources = new Array<Camera>()
      garbageStations.map(x => {
        this.tableService.cameras.filter(r => r.GarbageStationId == x.Id).map(c => {
          resources.push(c);
        });
      });
  ...
  setSearchDivision(){
    ...
    else this.tableService.search.divisionId =this.divisionBusinessService.divisionsId;
  }
  async ngOnInit() {  
    this.divisionDao.allDivisions().then(t=>{
      ...
      this.initTableList(); 
      this.tableService.divisionListView.toLevelListPanel(t.filter(x => x.ParentId != null));
    });
