...
import { FieldDesc, TableField } from "./station-view-summary/garbage-full-history-sum-chart/business/statistic-table";

export interface ICsvFieldVal{}

abstract class HWExportCsvStrategy {
  abstract export(title:string,fieldName:Array<string>,fieldVal:ICsvFieldVal): void;
}
...
export class StationSumHistoryCsv extends HWExportCsvStrategy {
  ...
  export(title:string,fieldName:Array<string>,fieldVal:ICsvFieldVal) {
    const csvDataMap = new Map<string, Array<string>>(),toFieldVal = (fieldStr: string) => {
      const field: FieldDesc = JSON.parse(fieldStr);
      return field.tdNumber+'';
    };
    var no = 1;
    csvDataMap.set(COLNAME, ['序号', '名称', '达标率', '平均落地时长', '最大落地时长', '总落地时长', '乱丢垃圾', '混合投放']);
    csvDataMap.set(TITLEKEY,[title]);
    (fieldVal as Array<TableField>).map((v: TableField) => {
      csvDataMap.set((no+''),[(no+''),v.name,toFieldVal(v.garbageRatio),toFieldVal(v.avgGarbageTime)
       ,toFieldVal(v.maxGarbageTime),toFieldVal(v.garbageRatio),toFieldVal(v.illegalDrop),toFieldVal(v.maxGarbageTime)]);
       no+=1;
    }); 
