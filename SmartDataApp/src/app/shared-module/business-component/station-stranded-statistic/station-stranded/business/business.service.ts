...
export class BusinessService {
    ...
    constructor(private datePipe: DatePipe
        ...
        this.galleryTargetView.neighborEventFnI = (ids, e: ImageEventEnum) => {
            const idV = ids.split('&'), findStation = this.stations.find(x => x.Id == idV[0]);
    ...
    async requestData(pageIndex: number, callBack?: (page: Page) => void) {
        const response = await this.garbageStationRequestService.statisticNumberList(this.getStationStatisticParam(pageIndex, this.search, this.toStationIds))
            .toPromise();
    ...
    getStationStatisticParam(pageIndex: number, search: SearchControl, stationIds: Array<string>) {
        const param = new GetGarbageStationStatisticNumbersParams(), day = TheDayTime(new Date());
