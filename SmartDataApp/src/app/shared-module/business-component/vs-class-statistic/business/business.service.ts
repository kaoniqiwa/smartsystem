...
export class BusinessService extends ListAttribute {
  ...
  convertBarData(
    ...
    const s = search.toSearchParam(),
      toPercent = (val: number, maxPercent: number, color: string) => {
        val = val || 0;
      ...
      toScore = (usedTime: number, color: string) => {
        ...
        return usedTime >= totalTime
          ? { percent: "100%", text: 0, color: color }
          : {
    ...
    const fillBarOpt = (
      ...
      if (s.TimeUnit == TimeUnitEnum.Hour) {
        opt[0] = toPercent(
          data.illegalDrop,
          data.illegalDrop + compare.illegalDrop,
          this.colors[0]
        );
        opt[1] = toPercent(
          data.mixedInto,
          data.mixedInto + compare.mixedInto,
          this.colors[0]
        );
        opt[2] = {
          percent: "1%",
        ...
        opt[3] = toPercent(
          data.dryVolume,
          parseFloat((data.wetVolume + compare.wetVolume).toFixed(2)),
          this.colors[0]
        );
        opt[4] = toPercent(
          data.wetVolume,
        ...
        opt[2] = {
          ...
          text: 0,
          color:
            data.fullDuration > compare.fullDuration
              ? this.colors[1]
              : this.colors[0],
        };
        opt[3] = toPercent(
      ...
      opt[0].color =
        data.illegalDrop > compare.illegalDrop
          ...
          : this.colors[0];
      opt[1].color =
        data.mixedInto > compare.mixedInto ? this.colors[1] : this.colors[0];
      opt[3].color =
        data.dryVolume > compare.dryVolume ? this.colors[1] : this.colors[0];
      opt[4].color =
        data.wetVolume > compare.wetVolume ? this.colors[1] : this.colors[0];
    };
    if (s.ClassType == ClassTypeEnum.Division) {
      const statistic_ = statistic as Array<DivisionNumberStatisticV2>,
        ...
        fillData = (
          data: Array<DivisionNumberStatisticV2>,
          barOptData: BarOptData
        ) => {
          data.map((m) => {
            m.EventNumbers.map((d) => {
              if (d.EventType == EventType.IllegalDrop)
                barOptData.illegalDrop += d.DayNumber;
              else if (d.EventType == EventType.MixedInto)
      ...
      const statistic_ = statistic as Array<GarbageStationNumberStatisticV2>,
        ...
        barOptDataG2 = new BarOptData(),
        fillData = (
          data: Array<GarbageStationNumberStatisticV2>,
          barOptData: BarOptData
        ) => {
          data.map((m) => {
            m.EventNumbers.map((d) => {
