...
export class BusinessService extends ListAttribute {
  ...
  initDivisionListView() {
    ...
    this.divisionListView.toLevelListPanel(
      this.divisions.filter((f) => f.ParentId != null)
    );
  }
    ...
    if (s.StationId) {
      ...
      if (_3dRequsetParam) {
        ...
        (_3dRequsetParam.searchParam as any[]).map(async (p) => {
          o.push(
            this.garbageStationService.eventNumbersHistory(
              p,
              _3dRequsetParam.stationId
            )
          );
        });
      ...
      const response = await this.divisionService.eventNumbersHistory(
        requsetParam.searchParam,
        requsetParam.divisionsId
      );
      mapData.set(s.DivisionId, response.Data);
      this.convertLineData(response.Data, this.search);
      this.convertBarData(response.Data, this.search);
      if (_3dRequsetParam) {
        const statistic = new Array<EventNumberStatistic>(),
        ...
        (_3dRequsetParam.searchParam as any[]).map(async (p) => {
          o.push(
            this.divisionService.eventNumbersHistory(
              p,
              _3dRequsetParam.divisionsId
            )
          );
        });
        forkJoin(o).subscribe((rs) => {
  ...
  convertBar3dData(statistic: EventNumberStatistic[], search: SearchControl) {
    ...
    if (
      ...
      for (let i = 0; i < 24; i++) this.bar3dOption.xAxis3dData.push(`${i}:00`);
      ...
      for (let i = 0; i < 7; i++) {
        ...
        for (let j = 0; j < 24; j++) {
          const findVal = statistic.find((s) => {
            ...
            return (
              `${parseInt(bg.getMonth() + 1 + "")}-${parseInt(
                bg.getDate() + ""
              )} ${parseInt(bg.getHours() + "")}` ==
              `${parseInt(week.monday.getMonth() + 1 + "")}-${parseInt(
                week.monday.getDate() + ""
              )} ${j}`
            );
          });
          if (findVal)
            convertEventData(
              ...
              arr.push([
                ...
                j,
                ed,
                `${this.bar3dOption.yAxis3dData[i]},${j}:00`,
              ]);
              this.bar3dOption.maxNumber =
                ed > this.bar3dOption.maxNumber
                  ? ed
                  : this.bar3dOption.maxNumber;
            });
          else
            arr.push([i, j, 0, `${this.bar3dOption.yAxis3dData[i]},${j}:00`]);
        }
  ...
  changeDatePicker() {
    const param = this.search.toSearchParam(),
      getDay = (month: string, day: string) => {
        const d = new Date();
