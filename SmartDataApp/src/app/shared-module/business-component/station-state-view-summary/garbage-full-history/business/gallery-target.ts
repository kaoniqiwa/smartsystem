...
export class GalleryTargetView {
    ...
    galleryTargetPageFn = (e: ImageEventEnum, id: string) => { 
        ...
        if (e == ImageEventEnum.next && page.next.item) {
            const enlargeImage = mp.getJPG(page.next.item.other.ImageUrl);
            this.galleryTarget = new GalleryTarget(null
                , null, enlargeImage, null
                , page.next.item.other.CameraId + '&' + page.next.item.event.EventId, this.toDownLoadImgName(page.next.item.event));         
        }
        else if (e == ImageEventEnum.prev && page.prev.item) {
            const enlargeImage = mp.getJPG(page.next.item.other.ImageUrl);
            this.galleryTarget = new GalleryTarget(null
                , null, enlargeImage, null
                , page.prev.item.other.CameraId  + '&' + page.prev.item.event.EventId, this.toDownLoadImgName(page.prev.item.event));          
        }
    ...
    initGalleryTarget(event:GarbageFullEventRecord,index:number) { 
        if (event&&event.Data&&event.Data.CameraImageUrls) {
            ...
            const enlargeImage = mp.getJPG(event.Data.CameraImageUrls[index].ImageUrl);         
            this.galleryTarget = new GalleryTarget(null
                , null, enlargeImage,
                null, event.Data.CameraImageUrls[index].CameraId+ '&' + event.EventId, this.toDownLoadImgName(event));
            this.galleryTarget.imgNext = (index + 1) < event.Data.CameraImageUrls.length;
