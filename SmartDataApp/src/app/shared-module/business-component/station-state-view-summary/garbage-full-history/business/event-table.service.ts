...
export class EventTableService extends ListAttribute {
    ...
    constructor(private eventRequestService: EventRequestService
        ...
        this.galleryTargetView.neighborEventFn = (ids, e: ImageEventEnum) => {
            const idV = ids.split('&'), gi = new GalleryTargetItem();
            var findEvent = this.dataSource.find(x => x.EventId == idV[1])
                , index = findEvent.Data.CameraImageUrls.findIndex(x => x.CameraId == idV[0]);
            gi.event = findEvent;
            ...
            if (e == ImageEventEnum.none) {
                ...
                if (index == findEvent.Data.CameraImageUrls.length - 1)
                    next = false;
                return {
                    item: gi,
                    prev: prev,
        ...
        this.eventTable.playVideosFn = async (ids) => {
            const idV = ids.split('&'), mp = new MediumPicture();
            var event = this.dataSource.find(x => {
                if (idV.length == 1)
                    return x.EventId == idV[0];
                else return x.EventId == idV[1];
            });

            if (event == null) event = this.allDataSource.find(x => x.EventId == idV[1]);
            if (event.Data) {
        ...
        this.videoFileFn = (ids) => {
            const idV = ids.split('&'), user = new SessionUser();
            ...
            this.garbageStationService.cameraFileUrl(event.Data.StationId, idV[0], s, e).subscribe(video => {
    ...
    appendVideoList(id: string) {
        ...
        if (event.Data) {
            const station = this.garbageStations.find(g => g.Id == event.Data.StationId);
        ...
        this.videoDownLoad.map(v => {
            if (v.state) {
                const event = this.eventTable.findEventFn(v.eventId),
                    ...
                    e = DateInterval(event.EventTime + '', user.video.afterInterval).toISOString();
                new MessageBar().response_success('正在下载中...');
                this.garbageStationService.cameraFileUrl(event.Data.StationId, v.cameraId, s, e).subscribe(video => {
                    const a = document.createElement('a');
                    a.href = video.Data.Url;
                    a.click();
                    document.body.appendChild(a);
    ...
    getVodUrlParam(eventId: string, cameraId: string, bgTime?: string, edTime?: string) {
    ...
    getCameraName(eventId: string, cameraId: string) {
        ...
        var name = '';
        if (event.Data) {
            const camera = event.Data.CameraImageUrls.find(x => x.CameraId == cameraId);
    ...
    toGarbageFullEventsRecord(list: Array<GarbageFullEventRecord>) {
        ...
        list.map(event => {
            if (event.Data) {
                const station = this.garbageStations.find(g => g.Id == event.Data.StationId),
                    camera = station.Cameras.filter(x => eh.cameraUsage.garbageFull.indexOf(x.CameraUsage) > 0),
                    delIndex = new Array<number>();
                for (let i = 0; i < event.Data.CameraImageUrls.length; i++) {
                    const index = camera.findIndex(x => x.Id == event.Data.CameraImageUrls[i].CameraId);
    ...
    async requestData(pageIndex: number, callBack?: (page: Page) => void) {
        if (this.search.state == false) {
            ...
            this.toGarbageFullEventsRecord(response.Data.Data);
