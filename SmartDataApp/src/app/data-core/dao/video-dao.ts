...
export class HWVideoService {
  ...
  async videoUrl(param: GetPreviewUrlParams | GetVodUrlParams) {
    var result: VideoUrl;
    const vu = new VideoUrl(),
      user = new SessionUser(),
      config = await this.userDalService.getUserConfig(
        user.id,
    ...
    if (param instanceof GetPreviewUrlParams) {
      ...
      param.BeginTime = DateInterval(
        param.BeginTime + "",
        user.video.beforeInterval
      );
      param.EndTime = DateInterval(
        param.EndTime + "",
    ...
    result.Url =
      result.Url.indexOf("password") > 0
        ? result.Url
        : result.Url + user.videoUserPwd;
    vu.Url = result.Url;
